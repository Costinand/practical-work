            ...
            with open(
                ...
                for row in csv_reader:  # перебор строки документа
                    ...
                    for (
                        key,
                        possible_keys,
                    ) in key_mapping.items():  # проверка содержимого строки
                    ...
                    self.data.append(
                        data
                    )  # заполнение нового прайс-листа на основе представленных
...
def search_product(self, search_query):
    ...
    sorted_results = sorted(
        results, key=lambda x: float(x.get("цена", 0)) / float(x.get("вес", 1))
    )
...
def export_to_html(
    self, output_file_path=r"C:\\Users\\radag\\Work_20\\Practice\\output.html"
):
    if self.data:
        sorted_data = sorted(
            self.data, key=lambda x: float(x.get("цена", 0)) / float(x.get("вес", 1))
        )
    ...
    while True:
        ...
        if results:
            ...
            for id, result in enumerate(sorted_results, 1):
                print(
                    "%1s" % id,
                    "%20s" % result.get("название"),
                    "%5s" % result.get("цена"),
                    "%3s" % result.get("вес"),
                    "%13s" % result.get("файл"),
                    "%9s"
                    % (float(result.get("цена", 0)) / float(result.get("вес", 1))),
                )
